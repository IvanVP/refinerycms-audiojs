module Refinery
  module Audios
    ...
    self.dragonfly_url_format = '/system/audios/:job/:basename.:format'
    self.trust_file_extensions = false
    self.max_file_size = 524288000
    self.pages_per_dialog = 7
    self.pages_per_admin_index = 20
    self.whitelisted_mime_types = %w(audio/mpeg)
    self.skin_css_class = "default-skin"

    class << self
      def datastore_root_path
        config.datastore_root_path || (Rails.root.join('public', 'system', 'refinery', 'audios').to_s if Rails.root)
      end
